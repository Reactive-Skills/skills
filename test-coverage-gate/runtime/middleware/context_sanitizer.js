...
function sanitizeObject(value) {
  ...
  if (typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([key, item]) => [/token|key|secret|password/i.test(key) ? key : key, /token|key|secret|password/i.test(key) ? '[REDACTED]' : sanitizeObject(item)]));
  return value;
}
