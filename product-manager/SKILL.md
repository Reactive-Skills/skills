...
> - To start a new task, use selected runtime `invoke product-manager [--payload JSON]`.
> - To resume an active task, use selected runtime `state product-manager`.
...
> - To advance state, use selected runtime `emit product-manager <signal>`.
>
...
> Direct AXI uses `reactive-skills-axi state|emit product-manager`.
> Zero-install AXI uses `npx -y @reactive-skills/axi state|emit product-manager`.
...
> If the current job is terminal, run selected runtime `reset product-manager` or `invoke product-manager`.
>
> **STRICT INVARIANT**
