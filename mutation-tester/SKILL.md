...
> - To start a new task: Run `reactive-skills-axi invoke mutation-tester [--payload JSON]`
> - To resume an active task: Run `reactive-skills-axi state mutation-tester`
> - For named or parallel work: Keep the same `--job <job-id>` flag on every command.
> - To advance state: Run `reactive-skills-axi emit mutation-tester <signal>`
>
